...
from pulse_analysis.analysis import analyze_files, read_signal
...
def main() -> None:
    ...
    plots_count = len(signal_files)
    columns = 4
    rows = (plots_count + columns - 1) // columns
    figure, axes = plt.subplots(rows, columns, figsize=(16, 3 * rows), squeeze=False)

    for index, path in enumerate(signal_files):
        signal, sampling_frequency = read_signal(
            path,
    ...
    figure.suptitle("Исходные PPG-сигналы", fontsize=14)
    figure.tight_layout(rect=(0, 0, 1, 0.98))
    plt.show()

    fourier_frequencies, ssa_frequencies = analyze_files(
        files=signal_files,
        signal_length=SIGNAL_LENGTH,
